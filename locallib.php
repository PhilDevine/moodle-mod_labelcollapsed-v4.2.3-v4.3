...
function labelcollapsed_get_html_content(cm_info $cm) {
    ...
    $content .= html_writer::start_tag('h4', array(
        ...
        'style' => 'padding:0; margin-bottom:15px;  margin-top:8px;'));
    $secnum = $lc->labelsection;
    if ($secnum !== "") {
        $content .= html_writer::start_tag('span',
        array('style' => 'background-color:'.$lc->sectionbgcolor.';color:'.$lc->sectioncolor.';margin-right: 8px;padding:4px 5px 4px 5px;border-radius: 6px 6px 6px 6px;'
        ));
